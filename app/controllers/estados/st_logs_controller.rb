class StLogsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @st_log.save
        format.html { redirect_to st_log_url(@st_log), notice: "St log was successfully created." }
  ...
  def update
    respond_to do |format|
      if @st_log.update(st_log_params)
        format.html { redirect_to st_log_url(@st_log), notice: "St log was successfully updated." }
  ...
  private
    ...
    def st_log_params
      params.require(:st_log).permit(:app_perfil_id, :class_name, :objeto_id, :e_origen, :e_destino)
    end
end
